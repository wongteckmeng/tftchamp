...
async def list_matches(request: Request, platform: Platform = 'na1', pagination: Tuple[int, int] = Depends(pagination)):
...
async def update_match(id: str, request: Request, match: MatchDetailUpdate = Body(...)):
    ...
    if (
        existing_match := await request.app.database[f"oc1_matches_detail"].find_one({"_id": id})
    ) is not None:
