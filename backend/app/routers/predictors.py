...
from models.predictor import MongoBaseModel, MetadataOutput, Predictor, PredictionInput, PredictionOutput, Image, ImagesList, Text, FeatureImportanceOutput, get_model
...
@model_router.get("/metadata", response_description="get latest metadata", response_model=MetadataOutput)
async def get_metadata() -> MetadataOutput:
    responses: MetadataOutput = {
        'latest_version': settings.latest_release, 'latest_patch': settings.patch.strftime("%Y-%m-%d")}
    return responses


@model_router.get("/image/{id}", response_description="Get a single match by id",
...
async def find_image(id: str, request: Request, platform: Platform = 'na1', league: League = 'challengers', version: str = settings.latest_release, patch: str = settings.patch.strftime("%Y-%m-%d")):
    # print(request.app.config)pickle.loads(
    if (binary := await request.app.database[f"{platform}_{league}_{version}_{patch}_binary"].find_one({"_id": id})) is not None:
        im = binary["image"]
...
async def list_images(request: Request, platform: Platform = 'na1', league: League = 'challengers', version: str = settings.latest_release, patch: str = settings.patch.strftime("%Y-%m-%d")):
