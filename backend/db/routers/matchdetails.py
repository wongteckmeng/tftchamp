...
async def create_match(request: Request, match: MatchDetail = Body(...)):
    ...
    new_match = request.app.database[f"oc1_matches_detail"].insert_one(match)
    created_match = request.app.database[f"oc1_matcehs_detail"].find_one(
...
async def list_matches(request: Request):
    matches = list(request.app.database[f"oc1_matches_detail"].find(limit=5))
    return matches
...
async def update_match(id: str, request: Request, match: MatchDetailUpdate = Body(...)):
    ...
    if len(match) >= 1:
        update_result = request.app.database[f"oc1_matches_detail"].update_one(
            {"_id": id}, {"$set": match}
        )
    ...
    if (
        existing_match := request.app.database[f"oc1_matches_detail"].find_one({"_id": id})
    ) is not None:
...
async def delete_match(id: str, request: Request, response: Response):
    delete_result = request.app.database[f"oc1_matches_detail"].delete_one({
                                                                           "_id": id})
